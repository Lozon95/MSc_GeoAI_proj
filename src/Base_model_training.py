...
# Build the model and fit the data, epochs, weights and callbacks 
...
history_finetune = model.fit(X_train, y_train, epochs=10, validation_data=(X_val, y_val), class_weight=class_weights, callbacks=[early_stopping])

# Could not access the training history in the saved model, 
# save separaelty to be able to print later in process if needed
import json
with open('TLVS_5_training_history_ONLYCLAYANDALLOVINUAL.json', 'w') as f:
    json.dump(history.history, f)

# Save model
model.save('Transfer_Learning_vgg16_SOIL_5_ONLYCLAYANDALLOVINUAL.keras')
